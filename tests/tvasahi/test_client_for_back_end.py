...
from tests.testlibraries.instance_resource import (
    ...
    COOKIES_LOGIN_EXAMPLE,
    COOKIES_STREAMING_EXAMPLE,
    EPISODE_EXAMPLE,
    EXTRA_LARGE_IMAGE_URL_EXAMPLE,
    FILE_HTML_EPISODE,
    FILE_HTML_PROGRAM_HOME,
...
class TestClientForBackEnd:
    ...
    def test_request_program_home_page(requests_mock):
        ...
        assert ClientForBackEnd.request_program_home_page(program_id_string) == BeautifulSoup(
            html_episode, "html.parser"
        )
    ...
    def test_request_api_list_episode(requests_mock):
        ...
        requests_mock.get(
            "https://mv.tv-asahi.co.jp/douga/api/list/episodes?"
            + "&".join([f"{key}={value}" for key, value in params.items()]),
            request_headers={"Referer": f"https://mv.tv-asahi.co.jp/douga/{program_id_string}/episodes"},
            json=json_example,
        )
        assert (
            ClientForBackEnd.request_api_list_episode(program_id_string, Payload(program_code, limit, sort))
            == json_example
        )
    ...
    def test_ffmpeg(mocked_run):
        ...
        mocked_run.assert_called_once_with(
            StreamFactory.create_stream_expect(COOKIES_STREAMING_EXAMPLE, PATH_FILE, URI_EXAMPLE)
        )

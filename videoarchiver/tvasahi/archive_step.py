...
class ArchiveStep(MembershipSiteArchiveStep):
    ...
    def __init__(
        self, client_for_back_end: Type[ClientForBackEnd], path_directory_download: Path, path_directory_cache: Path,
    ):
    ...
    def execute_per_program(self, program_id_string: str) -> None:
        ...
        list_episode = (
            self.episode_loader.list_up_episode(program_id_string)
            if len(sys.argv) >= 1 and "--no-cache" in sys.argv
            else self.episode_loader.load_backup(program_id_string)
        )
